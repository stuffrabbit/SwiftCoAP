...
class SCClient: NSObject {
    ...
    func sendWithRentransmissionHandling() {
        println("Retransmission Handling \(retransmissionCounter)")
        sendPendingMessage()
    ...
    private func sendHttpMessageFromCoAPMessage(message: SCMessage) {
        ...
        NSURLConnection.sendAsynchronousRequest(urlRequest, queue: NSOperationQueue.mainQueue()) { (response, data, error) -> Void in
            if error != nil {
                ...
                if coapResponse.options[SCOption.Observe.rawValue] == nil {
                    self.handleBlock2WithMessage(coapResponse)
                }
                else {
                    self.isMessageInTransmission = false
                }
            }
        }
    }
...
extension SCClient: GCDAsyncUdpSocketDelegate {
    func udpSocket(sock: GCDAsyncUdpSocket!, didReceiveData data: NSData!, fromAddress address: NSData!, withFilterContext filterContext: AnyObject!) {
        if let message = SCMessage.fromData(data) {
            ...
            //Handle Caching, Block, Separate, etc
            if cachingActive && messageInTransmission.code == SCCodeValue(classValue: 0, detailValue: 01) {
                ...
                println("Caching, amount: \(cachedMessagePairs.count)")
            }
            
            //Notify Delegate
            delegate?.swiftCoapClient(self, didReceiveMessage: message)
            
            //Handle Block2 or Observe-Option
            if let observeValue = message.options[SCOption.Observe.rawValue] {
                ...
                if recentNotificationInfo == nil ||
                   ...
                   (recentNotificationInfo.0 .compare(message.timeStamp!) == .OrderedAscending) {
